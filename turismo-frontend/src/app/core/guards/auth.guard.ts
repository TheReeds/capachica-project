...
import { Observable, of, map, catchError, take, tap } from 'rxjs';
...
export const authGuard: CanActivateFn = (route, state) => {
  ...
  // Verificar si el usuario ya seleccionó un panel (para rutas de dashboard)
  if (state.url.startsWith('/dashboard') && !authService.isPanelSelected()) {
    console.log('AuthGuard: Redirigiendo a selección de panel');
  ...
  if (authService.getToken()) {
    ...
    return authService.loadUserProfile(true).pipe(
      ...
      map(user => {
        if (user) {
          // Verificar nuevamente si se requiere selección de panel
          if (state.url.startsWith('/dashboard') && !authService.isPanelSelected()) {
            console.log('AuthGuard: Usuario autenticado pero sin panel seleccionado');
            return router.createUrlTree(['/seleccion-panel']);
          }
