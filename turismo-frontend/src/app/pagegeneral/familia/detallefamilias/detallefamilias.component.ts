...
export class DetallefamiliasComponent implements OnInit {
  ...
  // NUEVO: Propiedades para gestionar reseñas
  public resenas: any[] = [];
  public cargandoResenas: boolean = false;
  ...
  // NUEVO: Método para cargar reseñas del emprendedor
  cargarResenas() {
    this.cargandoResenas = true;
    this.http.get<{ data: any[] }>(`${environment.apiUrl}/resenas/emprendedor/${this.id}`).subscribe({
      next: (resp: { data: any[] }) => {
        this.resenas = Array.isArray(resp?.data) ? resp.data : [];
