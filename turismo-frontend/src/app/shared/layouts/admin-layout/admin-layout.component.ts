    ...
    <div class="flex h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      ...
      <div *ngIf="authService.isLoading()" 
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg flex items-center space-x-4">
          ...
          <span class="dark:text-white">Cargando tu perfil...</span>
        </div>
      </div>
      
      <!-- Sidebar -->
      <div 
        class="fixed inset-y-0 left-0 z-30 transform bg-gradient-to-b from-orange-700 to-orange-900 transition-all duration-300 ease-in-out shadow-xl"
        [ngClass]="{
      ...
      >
        <div class="flex h-16 items-center justify-between bg-orange-900 px-4">
          <div class="flex items-center space-x-2 overflow-hidden">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-orange-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="px-4 py-5 border-b border-orange-600/30">
          <div class="flex items-center">
            <!-- User Photo or Initials -->
            <div class="relative">
              <div *ngIf="profilePhotoUrl()" class="h-12 w-12 rounded-full overflow-hidden border-2 border-orange-300 shadow-md flex-shrink-0">
                <img [src]="profilePhotoUrl()" alt="Foto de perfil" class="h-full w-full object-cover" />
              </div>
              <div *ngIf="!profilePhotoUrl()" class="h-12 w-12 rounded-full bg-orange-100 flex items-center justify-center shadow-md flex-shrink-0">
                <span class="text-orange-700 font-medium text-sm">{{ userInitials() }}</span>
              </div>
              <div class="absolute bottom-0 right-0 h-3 w-3 rounded-full bg-green-500 border border-white"></div>
            </div>
            ...
            <div *ngIf="!sidebarCollapsed()" class="ml-3 overflow-hidden transition-opacity duration-200">
              ...
              <p class="text-xs text-orange-200 truncate">{{ userRole() }}</p>
            </div>
          </div>
          
          <!-- Toolbar (only when not collapsed) -->
          <div *ngIf="!sidebarCollapsed()" class="mt-4 flex justify-between transition-opacity duration-200">
            <button (click)="toggleSidebarCollapse()" class="text-orange-200 hover:text-white transition-colors" title="Minimizar menú">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7" />
              </svg>
            </button>
            <button (click)="toggleDarkMode()" class="text-orange-200 hover:text-white transition-colors" [title]="darkMode() ? 'Modo claro' : 'Modo oscuro'">
          ...
          <div *ngIf="sidebarCollapsed()" class="mt-4 flex flex-col items-center space-y-4 transition-opacity duration-200">
            <button (click)="toggleSidebarCollapse()" class="text-orange-200 hover:text-white transition-colors" title="Expandir menú">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="h-[calc(100vh-9rem)] overflow-y-auto py-4 px-3 scrollbar-thin scrollbar-thumb-orange-400 scrollbar-track-orange-100">
          <ul class="space-y-1 text-white">
            ...
            <li class="mt-5 pt-3 border-t border-orange-600/30" [class.text-center]="sidebarCollapsed()">
              <h3 class="px-4 text-xs font-bold text-orange-200 uppercase tracking-wider truncate" [class.opacity-0]="sidebarCollapsed()" [class.hidden]="sidebarCollapsed()">Usuarios y Acceso</h3>
              <div *ngIf="sidebarCollapsed()" class="h-px w-10 bg-orange-600/30 mx-auto"></div>
            </li>
            <li>
              <a 
            ...
            <!-- Sección Turismo -->
            <li class="mt-5 pt-3 border-t border-orange-600/30" [class.text-center]="sidebarCollapsed()">
              <h3 class="px-4 text-xs font-bold text-orange-200 uppercase tracking-wider truncate" [class.opacity-0]="sidebarCollapsed()" [class.hidden]="sidebarCollapsed()">Sistema de Turismo</h3>
              <div *ngIf="sidebarCollapsed()" class="h-px w-10 bg-orange-600/30 mx-auto"></div>
            </li>
            <li>
              <a 
            ...
            <!-- Sección Usuario -->
            <li class="mt-5 pt-3 border-t border-orange-600/30" [class.text-center]="sidebarCollapsed()">
              <h3 class="px-4 text-xs font-bold text-orange-200 uppercase tracking-wider truncate" [class.opacity-0]="sidebarCollapsed()" [class.hidden]="sidebarCollapsed()">Mi cuenta</h3>
              <div *ngIf="sidebarCollapsed()" class="h-px w-10 bg-orange-600/30 mx-auto"></div>
            </li>
            <li>
              <a 
          ...
          <div class="px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between">
              <div class="flex items-center lg:hidden">
                <button 
                  (click)="toggleSidebar()" 
                  class="text-gray-700 dark:text-gray-200 focus:outline-none hover:text-orange-500 transition-colors"
                >
                  <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="flex items-center space-x-4">
                ...
                <div class="relative group">
                  <div class="flex items-center cursor-pointer">
                    ...
                    <div *ngIf="profilePhotoUrl()" class="h-9 w-9 rounded-full overflow-hidden bg-orange-100 shadow-sm hover:shadow transition-shadow">
                      <img [src]="profilePhotoUrl()" alt="Foto de perfil" class="h-full w-full object-cover" />
                    </div>
                    <div *ngIf="!profilePhotoUrl()" class="h-9 w-9 rounded-full bg-orange-100 flex items-center justify-center shadow-sm hover:shadow transition-shadow">
                      <span class="text-orange-700 font-medium">{{ userInitials() }}</span>
                    </div>
                  </div>
                  
                  <!-- Dropdown Menu -->
                  <div class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none hidden group-hover:block transition-all duration-200">
                    ...
                    <button (click)="logout()" class="w-full text-left block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">Cerrar Sesión</button>
                  </div>
                </div>
              </div>
            </div>
...
export class AdminLayoutComponent implements OnInit {
  ...
  private loadThemePreference() {
    ...
    if (savedDarkMode === 'true') {
      // Si hay una preferencia explícita de modo oscuro, aplicarla
      this.darkMode.set(true);
      console.log('Aplicando modo oscuro desde localStorage');
    } else if (savedDarkMode === 'false') {
      // Si hay una preferencia explícita de modo claro, aplicarla
      this.darkMode.set(false);
      console.log('Aplicando modo claro desde localStorage');
    } else if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
      // Si no hay preferencia guardada, usar la preferencia del sistema
      this.darkMode.set(true);
      console.log('Aplicando modo oscuro desde preferencia del sistema');
    }
    
    // Aplicar la clase 'dark' al documento según el estado actual
    if (this.darkMode()) {
      document.documentElement.classList.add('dark');
    } else {
      document.documentElement.classList.remove('dark');
    ...
    // Recuperar estado de colapso del sidebar
